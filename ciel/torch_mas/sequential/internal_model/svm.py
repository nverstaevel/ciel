...
class SVM(InternalModelInterface):
    ...
    def update(self, X, y, agent_mask):
        ...
        base_prediction = torch.zeros(
            self.base_prediction[agent_mask].size(0),
            dtype=torch.float,
            device=self.device,
        )
        models = torch.zeros(
            (self.models[agent_mask].size(0), X.size(2) + 1, 1), device=self.device
        )

        has_different_classes = ((y == -1).any(dim=1) & (y == 1).any(dim=1)).reshape(
            X.size(0)
        )
        ...
        if has_different_classes.sum() > 0:
            ...
            # update agents
            updated_models = batch_fit_linear_svm(X_train, y_train, device=self.device)
